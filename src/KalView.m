...
static const CGFloat kHeaderHeight = 78.f;
static const CGFloat kMonthLabelHeight = 17.f;

@implementation KalView

- (id)initWithFrame:(CGRect)frame delegate:(id <KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic colorSource:(KalColorSource *)source {
    colorSource = source;
...
- (void)addSubviewsToHeaderView:(UIView *)headerView {
    ...
    const CGFloat kHeaderVerticalAdjust = 16.f;
    const CGFloat kMonthButtonVerticalAdjust = -6.f;

    // Create the previous month button on the left side of the view
    CGRect previousMonthButtonFrame = CGRectMake(self.left,
            kHeaderVerticalAdjust + kMonthButtonVerticalAdjust,
    ...
    headerTitleLabel.textColor = colorSource.normalColor;
    ...
    NSArray *fullWeekdayNames = [formatter standaloneWeekdaySymbols];
    ...
    for (CGFloat xOffset = 0.f; xOffset < headerView.width; xOffset += 46.f, i = (i + 1) % 7) {
        ...
        weekdayLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:15];
        weekdayLabel.textAlignment = UITextAlignmentCenter;
        weekdayLabel.textColor = colorSource.normalColor;
        weekdayLabel.text = [weekdayNames[i] capitalizedString];
...
- (void)addSubviewsToContentView:(UIView *)contentView {
    ...
    self.gridView = [[KalGridView alloc] initWithFrame:fullWidthAutomaticLayoutFrame logic:logic delegate:self.delegate colorSource:colorSource];
